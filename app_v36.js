...
// ===================== CENOVÉ TABULKY (orientační, vč. DPH) =====================
// Cena za m² podle "Cenové skupiny látky" (1-4, u plisé a vertikál 1-6)
const PRICE_M2={
  stresni:[8797,9166,9687,10166],
  aluvegasdn:[4909,5352,6389,7596],
  maxidn:[5247,5635,6507,8061],
  otevrenadn:[4279,4827,5783,6861],
  polodn:[4594,5247,5783,6861],
  sonosdn:[5142,5531,6507,8061],
  sonolldn:[5142,5531,6507,8061],
  aluvegastx:[3115,3568,4078,4198],
  maxitx:[4832,5215,6087,6906],
  otevrenatx:[3116,3643,4185,4690],
  polotx:[4062,4618,5098,5593],
  sonostx:[3320,3635,4256,4675],
  sonolltx:[3320,3635,4256,4675],
  plise:[5487,6062,6577,9061,9252,11529],
  // Vertikální žaluzie LUX — zjednodušeno na 6 cenových skupin (orientační průměr z ceníku)
  vertikal:[1700,1900,2050,2200,2500,3050],
};

// Horizontální žaluzie — cena za m² podle Typ žaluzie × Typ lamely
// POZN: ceník pro ISOLINE LOCO chybí — používá se prozatím cena ISOLINE ECO jako náhrada.
const PRICE_HORIZONT={
  'PRIM':{'25 x 0,18':1961,'25 x 0,21':2746,'16 x 0,21':2905},
  'ECO':{'25 x 0,18':1974,'25 x 0,21':2763,'16 x 0,21':2922},
  'ISOLINE LOCO':{'25 x 0,18':1974,'25 x 0,21':2763,'16 x 0,21':2922}, // TODO: nahradit reálným ceníkem LOCO
};

// Pevná okenní síť — cena za m² podle profilu (jen profily se známou cenou)
const PRICE_SIT_PEVNA={
  'OE 32x11 LUX':5366,
  'ISSO OE 19x8':2311,
  'ISSO OE 25x10':2609,
};

// Přidání pole "Cenová skupina látky" ke všem produktům s látkou
const FABRIC_4=['stresni','aluvegasdn','maxidn','otevrenadn','polodn','sonosdn','sonolldn','aluvegastx','maxitx','otevrenatx','polotx','sonostx','sonolltx'];
const FABRIC_6=['plise','vertikal'];
FABRIC_4.forEach(key=>{PRODUCTS[key].fields.push({id:'cenova_skupina',label:'Cenová skupina látky',type:'select',required:true,options:['1','2','3','4'],defaultVal:'1',hint:'Dle ceníku — vyšší číslo = dražší látka'});});
FABRIC_6.forEach(key=>{PRODUCTS[key].fields.push({id:'cenova_skupina',label:'Cenová skupina látky',type:'select',required:true,options:['1','2','3','4','5','6'],defaultVal:'1',hint:'Dle ceníku — vyšší číslo = dražší látka'});});

function formatKc(n){return Math.round(n).toString().replace(/\B(?=(\d{3})+(?!\d))/g,' ')+' Kč';}

// Spočítá orientační cenu položky (bez příplatků). Vrací null pokud cenu nelze spočítat.
function calcPrice(o){
  ...
  let pricePerM2=null;
  if(PRICE_M2[o._typ]){
    const idx=Math.max(0,(+o.cenova_skupina||1)-1);
    pricePerM2=PRICE_M2[o._typ][idx];
  } else if(o._typ==='horizont'){
    const tbl=PRICE_HORIZONT[o.typ_zal];
    if(tbl)pricePerM2=tbl[o.typ_lamely];
  } else if(o._typ==='sit_dverpant'){
    const drahe=/(mahagon|ořech|orech|zlat[yý] dub)/i.test(o.bprofilu||'');
    pricePerM2=drahe?5986:5513;
  } else if(o._typ==='sit_posuvram'){
    pricePerM2=5196;
  } else if(o._typ==='sit_posuvlist'){
    pricePerM2=5172;
  } else if(o._typ==='sit_rolovaci'){
    pricePerM2=6118;
  } else if(o._typ==='sit_pevna'){
    pricePerM2=PRICE_SIT_PEVNA[o.profil]||null;
  }
  if(pricePerM2==null)return null;
  const plocha=prod.getM2?prod.getM2(o):0;
  if(!plocha)return null;
  return Math.round(pricePerM2*plocha*pocet);
}

// ===================== STATE =====================
const STORE='zamereni-stav';
let items=[],activTyp=null;
function saveState(){try{localStorage.setItem(STORE,JSON.stringify({klient:g('klient'),zakazka:g('zakazka'),adresa:g('adresa'),datum:g('datum'),technik:g('technik'),techniktel:g('technik-tel'),sleva:g('sleva'),items,activTyp}));}catch(e){}}
function loadState(){try{const raw=localStorage.getItem(STORE);if(!raw)return;const s=JSON.parse(raw);['klient','zakazka','adresa','datum','technik'].forEach(id=>{if(s[id])document.getElementById(id).value=s[id];});if(s.techniktel){const el=document.getElementById('technik-tel');if(el)el.value=s.techniktel;}if(s.sleva){const sl=document.getElementById('sleva');if(sl)sl.value=s.sleva;}if(s.items)items=s.items;if(s.activTyp)activTyp=s.activTyp;}catch(e){}}
window.resetState=function(){if(!confirm('Smazat zakázku a začít novou?\n(Jméno a telefon technika zůstanou)'))return;try{localStorage.removeItem(STORE);}catch(e){}items=[];activTyp=null;['klient','zakazka','adresa'].forEach(id=>document.getElementById(id).value='');document.getElementById('datum').value=new Date().toISOString().split('T')[0];var slevaEl=document.getElementById('sleva');if(slevaEl)slevaEl.value='';var sel=document.getElementById('typ-select');if(sel)sel.value='';document.getElementById('form-box').style.display='none';saveState();renderList();};
...
function renderForm(typ){
  ...
  const REMEMBER=['mis','ovladani','bprofilu','typ_latky','kod_latky','uchyceni','vedeni','boc_lista','spod_lista','vod_lista','vynaseci','mont_lprofil','bsitoviny','barva_box','typ_stah','provedeni','sirka_latky','kartacek','sitovina','typ_pantu','magnet','bsite','typ_site','prah','montaz','typ_montaze','typ_dorazu','brzda','profil','typ_profil','cenova_skupina'];
...
document.getElementById('btn-pdf').addEventListener('click',function(){
  ...
  const sleva=Math.min(100,Math.max(0,+g('sleva')||0));

  // Seskup položky podle typu
  const order=[],groups={};
  items.forEach(o=>{if(!groups[o._typ]){groups[o._typ]=[];order.push(o._typ);}groups[o._typ].push(o);});

  // Vytvoř řádky tabulky — místnost, typ produktu, počet a orientační cena (BEZ rozměrů)
  let tableRows='';
  let radek=1;
  let totalPrice=0;
  ...
  order.forEach(typ=>{
    ...
    groups[typ].forEach(o=>{
      ...
      const cena=calcPrice(o);
      let cenaTxt;
      if(cena!=null){totalPrice+=cena;cenaTxt=formatKc(cena);}
      else{cenaTxt='individuálně';hasUnpriced=true;}
      tableRows+=`<tr style="background:${bg}">
        ...
        <td style="padding:10px 14px;border-bottom:1px solid #f0e0e0;font-size:13px;text-align:right;color:#1a1a1a;white-space:nowrap">${cenaTxt}</td>
      </tr>`;
      radek++;
    });
  });
...
<div class="page">
  ...
  <table>
    <thead><tr>
      ...
      <th>Typ produktu</th>
      <th style="width:60px;text-align:center">Počet</th>
      <th style="width:110px;text-align:right">Cena</th>
    </tr></thead>
    <tbody>${tableRows}</tbody>
  </table>

  <div style="display:flex;justify-content:flex-end;margin-top:14px">
    <div style="background:#fdecea;border-radius:8px;padding:12px 20px;text-align:right;min-width:240px">
      ${sleva>0?`<div style="font-size:10px;font-weight:700;color:#cb1419;text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px">Cena bez slevy</div>
      <div style="font-size:15px;color:#999;text-decoration:line-through;margin-bottom:6px">${formatKc(totalPrice)}</div>
      <div style="font-size:10px;font-weight:700;color:#cb1419;text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px">Cena se slevou ${sleva}%</div>
      <div style="font-size:24px;font-weight:700;color:#1a1a1a">${formatKc(totalPrice*(1-sleva/100))}</div>`
      :`<div style="font-size:10px;font-weight:700;color:#cb1419;text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px">Orientační cena celkem</div>
      <div style="font-size:24px;font-weight:700;color:#1a1a1a">${formatKc(totalPrice)}</div>`}
      ${hasUnpriced?'<div style="font-size:10px;color:#888;margin-top:3px">+ položky s cenou na míru</div>':''}
    </div>
  </div>
...
// ===================== INIT =====================
document.querySelectorAll('#klient,#zakazka,#adresa,#datum,#technik,#technik-tel,#sleva').forEach(function(el){el.addEventListener('input',saveState);});
